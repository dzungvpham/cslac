...
# Tag each unique course title with a subfield/meta category (adds a
# `category` column in-place to every data/course_schedule/*.csv). Uses the
# `claude` CLI (Sonnet 4.6, low effort) in batches; resumable via a
# data/course_classification.json cache. --overwrite reclassifies everything.
python course_classifier.py [--overwrite]
```
...
data/course_schedule/*.csv         → course_classifier.py                  → data/course_schedule/*.csv (in-place, adds `category` column)
```
...
At write time, two invariants are enforced. `carry_forward_urls` preserves each existing member's curated URL: for any `(name, college)` already in `faculty_list.csv` with a non-empty URL, the old URL is kept instead of the freshly-scraped one (preferring an exact `(name, title, college)` match), so a re-scrape never drops a URL or swaps a personal site for a directory page / the Google-fallback's occasional garbage — only brand-new faculty (or those whose prior URL was blank) take the scraped URL. `sort_faculty_list` then writes the file stably sorted by `(college, name, title)` (case-insensitive); the row-aligned siblings inherit this order because they rebuild in `faculty_list.csv` order. The `--college` merge path applies both incrementally: it carries URLs forward for the re-scraped college and sorts the replaced rows within their block (keeping the file globally sorted) while leaving every other college's row positions untouched so the siblings stay aligned.
...
The runner `scrape_course_schedule.py` instantiates every scraper in `SCRAPERS`, including `*selfservice_scrapers()`, `*banner9_scrapers()`, `*jenzabar_jics_scrapers()`, and `*powercampus_scrapers()`. By default each scraper skips `(academic_year, term)` pairs already present in its CSV and only fetches new ones; `--force` re-scrapes every configured pair (replacing existing rows for that pair while preserving pairs absent from the new scrape). For scrapers with `terms = []` (one URL per year covering all terms, e.g. Williams), any row matching the academic year counts as coverage and the year is skipped. Output columns: `college, academic_year, term, course_code, section, course_name, instructor, time, url`. The `category` column added by `course_classifier.py` is not produced by scraping but is preserved across re-writes (re-attached by normalized course title, so it survives `--force` too); newly seen titles are left blank for the next classifier run.

**`scraper/course_classifier.py`** — Post-processing pass over the course CSVs. Deduplicates course titles globally by a normalized key (NFKD + strip accents + collapse-whitespace + lowercase) so each distinct title is classified once regardless of college/term, then asks the `claude` CLI (`--model claude-sonnet-4-6 --effort low --output-format json`, classification rules + examples in a custom `--system-prompt`) to label each into one of the 32 `CS_SUBFIELD_NAMES` or a meta bucket — `Core` (intro/data-structures/discrete-math/computer-org/the-standard-algorithms-course; advanced/applied/specialized algorithms and theory-of-computation stay in `Algorithms & complexity`), `Misc` (independent study/research/thesis/seminar/lab), `Other` (CS elective with no matching subfield), `Unknown` (not CS, e.g. Calculus). Titles are sent in batches of 50 to amortize the fixed per-call context (a custom system prompt + `--disallowedTools <all>` + `--strict-mcp-config` minimize it); 4 workers with per-call backoff stay under the subscription rate limit. Resumable via a `data/course_classification.json` cache (normalized-title → category); failed calls are left blank and reported, never fabricated. Writes a `category` column back into every `data/course_schedule/*.csv` in-place.
...
- **Courses** — reads every CSV under `data/course_schedule/` and adds per-college `terms` and `courses` with sorted term columns (year then F→W→S→Su) and `(code, name)`-deduplicated course rows, restricted to academic years ≥ 2021-22. Codes that resolve to the same display name (cross-listings, renumberings, undergrad/grad pairs, section-suffix/dual-coding variants — e.g. Wesleyan `COMP112/COMP112Z`, New College `CAP 3328/CSCI 3200`) are then collapsed into one row under a single representative code (the one whose most-recent offering is latest), unioning their term coverage, per-cell URLs, and instructors; generic shared names (Independent Study, Laboratory, …) are already removed beforehand by `EXCLUDE_RE` and the lab-only/`L`-suffix drop. Instructor strings are matched against the in-memory faculty list to produce per-cell initials + link. Each course entry also carries a `category` (a CS subfield or `Core`/`Misc`/`Other`/`Unknown`, from the `category` column written by `course_classifier.py`), which the dashboard's subfield filter chips reuse to gate course visibility (`courseVisible()` in `app.js`) exactly as they gate faculty and publications.
