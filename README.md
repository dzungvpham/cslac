...
- **Publications** — per-paper detail from OpenAlex for every CS work co-authored by a tracked faculty member: title, venue, year, citation count, co-authors (with student / faculty / other-LAC / external classification), and venue rankings from CSRankings, [ICORE 2026](https://portal.core.edu.au/conf-ranks/), and [Scimago (SJR)](https://www.scimagojr.com/).
...
5. **Publication scraping.** We query OpenAlex for every CS work affiliated with each college (by ROR ID), match the authorships to our faculty list by `(last name, first initial)`, and classify the remaining co-authors as students, fellow faculty, other-LAC collaborators, or external. Duplicate papers (same title in multiple venues) are merged, preferring the published version over preprints.
6. **Venue tagging.** Each paper's venue is matched against three external ranking sources: the [CSRankings](https://csrankings.org/) conference list (acronym + area), [ICORE 2026](https://portal.core.edu.au/conf-ranks/) (`A*/A/B/C` for conferences), and [Scimago Journal Rank](https://www.scimagojr.com/) (`Q1`–`Q4` for journals). The dashboard uses these to badge each paper.
7. **Course scraping.** A separate pipeline visits each college's course catalog and pulls every CS course offering for recent academic years. Different schools run very different catalog systems, so each school — or family of schools sharing a platform (Ellucian Colleague Self-Service, Banner 9, PowerCampus, Jenzabar JICS) — gets its own small scraper.
8. **Dashboard generation.** The CSVs from the steps above are merged into a single JSON file that powers the static dashboard published at cslac.org.
...
- [`data/faculty_publications.csv`](data/faculty_publications.csv) — one row per CS paper co-authored by a tracked faculty member, with venue, year, citation count, co-author classification, OpenAlex topics, and CSRankings / ICORE / Scimago venue rankings.
- [`data/course_schedule/<College Name>.csv`](data/course_schedule/) — one CSV per college with recent CS course offerings.

All four faculty CSVs are aligned by row order on `(name, title, college)` and can be joined directly. The publications and course-schedule CSVs join back to the faculty list by `(college, name)`.
